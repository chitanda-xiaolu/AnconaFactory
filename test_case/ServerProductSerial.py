...
class ServerProductSerial(TempItem):
    ...
    def exe(self):
        with self.ssh_connect(uut=self.config["UUT"]):
            _mes = MesSocket()
            #查看机头的三段码
            write_info =  _mes.get_mes_info(self.options.g_cfg.get_sn())["Results"]["server_sn"]
            ...
            self.assertEqual(f"Chassis Part Serial ", write_info, parser)
...
if __name__ == '__main__':
    TempRun.set_item(ServerProductSerial)
