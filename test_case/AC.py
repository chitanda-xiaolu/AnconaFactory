...
class CpldFwUpdate(TempItem):
    ...
    def tearDown(self):
        ...
        with self.ssh_connect(uut=self.config["UUT"]):
            parser = self.execute_run(f'''[[ `df | grep -iE "//192.2.19.51/share.*/mnt"` != '' ]] || mount -t cifs -o vers=2.0,username=Administrator,password=\`1q,sec=ntlmssp,cache=none,nobrl {path.get('source_path')} /mnt && echo 'mount success' ''')
            parser = self.execute_run(f"rpm -ivh {path.get('mount_path')}sshpass-1.09-4.el8.x86_64.rpm && echo 'create test dir success'")
            parser = self.execute_run(f"[ ! -d {path.get('fru_path')} ] && mkdir -p {path.get('fru_path')} && echo 'create test dir success'")
            parser = self.execute_run(f"rm -rf {path.get('fru_path')}/* && echo 'clear fru dir success'")
            parser = self.execute_run(f"cp -rf /mnt/fru/* {path.get('fru_path')} && echo 'copy fru file success'")
            parser = self.execute_run(f"[ ! -d {path.get('fw_path')} ] && mkdir -p {path.get('fw_path')} && echo 'create fw dir success'")
            parser = self.execute_run(f"rm -rf {path.get('fw_path')}/* && echo 'clear fw dir success'")
            parser = self.execute_run(f"\cp -rf {path['fw_source_path']} {path.get('fw_path')} && echo 'copy FW file success'")
            parser = self.execute_run(f"cp -rf {path['kingkong_source_path']} {path.get('test_path')} && echo 'Copy Kingkong tool success'")
            parser = self.execute_run("chmod -R 777 /opt/Alioam/")
            parser = self.execute_run(f'{path["oampower_read_script"]}')
