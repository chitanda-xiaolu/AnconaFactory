...
class ServerProductPartNumber(TempItem):
    ...
    def exe(self):
        with self.ssh_connect(uut=self.config["UUT"]):
            ...
            parser = self.execute_run(f"ipmitool fru edit 0 field p 2 {write_info}")
            data = self.execute_run("ipmitool fru print 0 ")
            parser = _mes.json_filter(data, "Product Part Number" )
            self.assertEqual(f"Product Part Number", write_info, parser)
        return Pass(self)


if __name__ == '__main__':
    TempRun.set_item(ServerProductPartNumber)
