...
class HibChassisExtra(TempItem):
    ...
    def exe(self):
        with self.ssh_connect(uut=self.config["UUT"]):
            ...
            _mes = MesSocket()
            write_info =  _mes.get_mes_info(self.options.g_cfg.get_sn())["Results"]["jbog_customer_part_number"]
            part_number = write_info if write_info != "TG88M1S1V5X7WS.03ZA.C0V1P4U8" else "TG88M1S1V5X7WS.03ZA.C0V1"
            ...
            parser = self.execute_run(f"ipmitool  -I lanplus -H {jbmc_ip} -U {jbmc_user} -P {jbmc_passwd} fru edit 0 field c 2 {write_info} ", i_exit_code=True)
            data = self.execute_run(f"ipmitool  -I lanplus -H {jbmc_ip} -U {jbmc_user} -P {jbmc_passwd} fru print 0 ", i_exit_code=True)
            parser = _mes.json_filter(data, "Chassis Extra" )
            self.assertEqual(f" Hib Chassis board Extra ", write_info, parser)
            # self.assertEqual(f"clear bmc sel log", int(1), len(count))
        return Pass(self)


if __name__ == '__main__':
    TempRun.set_item(HibChassisExtra)
