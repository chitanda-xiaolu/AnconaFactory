...
class HibBoardPartNumber(TempItem):
    ...
    def exe(self):
        with self.ssh_connect(uut=self.config["UUT"]):
            ...
            _mes = MesSocket()
            write_info =  _mes.get_mes_info(self.options.g_cfg.get_sn())["Results"]["hib_part_number"]
            data = self.execute_run(f"ipmitool  -I lanplus -H {jbmc_ip} -U {jbmc_user} -P {jbmc_passwd} fru print 0 ", i_exit_code=True)
            parser = self.execute_run(f"ipmitool  -I lanplus -H {jbmc_ip} -U {jbmc_user} -P {jbmc_passwd} fru edit 0 field b 3 {write_info} ", i_exit_code=True)
            data = self.execute_run(f"ipmitool  -I lanplus -H {jbmc_ip} -U {jbmc_user} -P {jbmc_passwd} fru print 0 ", i_exit_code=True)
            parser = _mes.json_filter(data, "Board Part Number" )
            self.assertEqual(f" Hib Chassis Board Part Number ", write_info, parser)
            # self.assertEqual(f"clear bmc sel log", int(1), len(count))
        return Pass(self)


if __name__ == '__main__':
    TempRun.set_item(HibBoardPartNumber)
