...
class FortyFcbChassisPartNumber(TempItem):
    ...
    def exe(self):
        with self.ssh_connect(uut=self.config["UUT"]):
            ...
            if part_number == "RK0037030004" or part_number == "RK0037030018":
                ...
                write_info =  _mes.get_mes_info(self.options.g_cfg.get_sn())["Results"]["sn_40"]
                write_info = "231A14089002"
                # 差一段三段码
                data = self.execute_run("ipmitool  -I lanplus -H %s -U %s -P %s  fru print 2 " % (jbmc_ip, jbmc_user ,jbmc_passwd), i_exit_code=True).data
                parser = self.execute_run(f"ipmitool -I lanplus -H {jbmc_ip} -U {jbmc_user} -P {jbmc_passwd} fru edit 2 field b 3 '{write_info}' ", i_exit_code=True)
                data = self.execute_run("ipmitool  -I lanplus -H %s -U %s -P %s  fru print 2 " % (jbmc_ip, jbmc_user ,jbmc_passwd), i_exit_code=True).data
                parser = _mes.json_filter(data,  "Part Number" )
                self.assertEqual(f"40 FB Part Number ", write_info, parser)
                # self.assertEqual(f"clear bmc sel log", int(1), len(count))
        return Pass(self)


if __name__ == '__main__':
    TempRun.set_item(FortyFcbChassisPartNumber)
