...
class FourNvmeBoardSerial(TempItem):
    ...
    def exe(self):
        with self.ssh_connect(uut=self.config["UUT"]):
            ...
            _mes = MesSocket()
            write_info =  _mes.get_mes_info(self.options.g_cfg.get_sn())["Results"]["nvme_board_sn"].split(',')
            ...
            self.assertEqual(f" Hib Chassis Board Serial ", write_info[0], parser)
            ...
            parser = _mes.json_filter(data, "Board Serial" )
            self.assertEqual(f" Hib Chassis Board Serial ", write_info[1], parser)
            # self.assertEqual(f"clear bmc sel log", int(1), len(count))
        return Pass(self)


if __name__ == '__main__':
    TempRun.set_item(FourNvmeBoardSerial)
